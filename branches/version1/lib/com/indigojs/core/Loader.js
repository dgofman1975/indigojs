...
.$define({
    static: {
        ...
        include: function() {
            ...
            var callBack, length = args.length;
    ...
    protected: {
        ...
        ready: function(_, cls) {
            Loader.classMap[cls].ready = true;
            var listeners = Loader.classMap[cls].listeners;
    ...
    public: {
        load: function(_, classes, callBack) {
            ...
            var complete = function(cls) {
                ...
                for (var i = 0; i < classes.length; i++)
                    map[classes[i]] = null;
                Indigo.inner.instances[self.uid] = null;
            ...
            for (var i = 0; i < classes.length; i++) {
                var cls = classes[i];
                if (!map[cls].ready) {
                    map[cls].listeners.unshift(complete);
